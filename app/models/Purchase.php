...
class Purchase extends Model {
    ...
    public function allByDateRange($start, $end) {
        $stmt = $this->db->prepare('SELECT p.*, s.name as supplier_name, s.tin_number as supplier_tin, pr.name as product_name FROM purchases p JOIN suppliers s ON p.supplier_id = s.supplier_id JOIN products pr ON p.product_id = pr.product_id WHERE p.purchase_date BETWEEN ? AND ? ORDER BY s.name, p.purchase_date DESC');
        $stmt->execute([$start, $end]);
        return $stmt->fetchAll();
    }

    public function allGroupedBySupplier($start, $end) {
        $stmt = $this->db->prepare('SELECT p.*, s.name as supplier_name, s.tin_number as supplier_tin, s.supplier_id, pr.name as product_name FROM purchases p JOIN suppliers s ON p.supplier_id = s.supplier_id JOIN products pr ON p.product_id = pr.product_id WHERE p.purchase_date BETWEEN ? AND ? ORDER BY s.name, p.purchase_date DESC');
        $stmt->execute([$start, $end]);
        ...
        foreach ($purchases as $p) {
            ...
            if (!isset($grouped[$supplier_id])) {
                $grouped[$supplier_id] = [
                    ...
                    'supplier_tin' => $p['supplier_tin'],
                    'supplier_id' => $supplier_id,
                    'purchases' => [],
                    'total' => 0
                ];
            }
            $grouped[$supplier_id]['purchases'][] = $p;
            $grouped[$supplier_id]['total'] += $p['total_price'];
        }
    ...
    public function find($id) {
        $stmt = $this->db->prepare('SELECT p.*, s.name as supplier_name, s.tin_number as supplier_tin, pr.name as product_name FROM purchases p JOIN suppliers s ON p.supplier_id = s.supplier_id JOIN products pr ON p.product_id = pr.product_id WHERE p.purchase_id = ?');
        $stmt->execute([$id]);
        return $stmt->fetch();
    }
