...
<style>
      ...
      body.dark-mode .stat-card.active {
        border-color: #6ea8fe;
        box-shadow: 0 8px 16px rgba(110,168,254,0.4) !important;
      }
      ...
      body.dark-mode .client-header-row {
        background-color: #1a3d4a !important;
        border-left-color: #6dd5ed !important;
      }
...
<div class="container mt-4">
  <div class="d-flex justify-content-between align-items-center mb-3">
    <h2 class="fw-bold text-primary mb-0">
      ...
      <small class="text-success fs-6" id="lastUpdated">Last updated: <span id="currentTime"></span></small>
    </h2>
  ...
  <div class="row g-3 mb-4">
    <div class="col-md-4">
      <div class="card border-0 shadow-sm h-100" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white;">
        <div class="card-body text-center">
          <div class="mb-2"><i class="fa fa-money-bill-wave fa-2x"></i></div>
          <h5 class="card-title">Total Amount Paid</h5>
          <div class="display-6 fw-bold"><?=number_format($totalPaid, 2)?></div>
          <p class="mb-0 opacity-75">RWF</p>
          <div class="progress mt-2" style="height: 6px; background: rgba(255,255,255,0.3);">
            <div class="progress-bar bg-light" style="width: <?=($totalAmount > 0) ? round(($totalPaid/$totalAmount)*100) : 0?>%"></div>
          </div>
          <small class="mt-2 d-block opacity-75">
            <?=($totalAmount > 0) ? round(($totalPaid/$totalAmount)*100) : 0?>% of total collected
          </small>
        </div>
      </div>
    </div>
    <div class="col-md-4">
      <div class="card border-0 shadow-sm h-100" style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); color: white;">
        <div class="card-body text-center">
          <div class="mb-2"><i class="fa fa-hand-holding-usd fa-2x"></i></div>
          <h5 class="card-title">Total Outstanding Balance</h5>
          <div class="display-6 fw-bold"><?=number_format($totalBalance, 2)?></div>
          <p class="mb-0 opacity-75">RWF</p>
          <div class="progress mt-2" style="height: 6px; background: rgba(255,255,255,0.3);">
            <div class="progress-bar bg-warning" style="width: <?=($totalAmount > 0) ? round(($totalBalance/$totalAmount)*100) : 0?>%"></div>
          </div>
          <small class="mt-2 d-block opacity-75">
            <?=($totalAmount > 0) ? round(($totalBalance/$totalAmount)*100) : 0?>% still pending
          </small>
        </div>
      </div>
    </div>
    <div class="col-md-4">
      <div class="card border-0 shadow-sm h-100" style="background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%); color: white;">
        <div class="card-body text-center">
          ...
          <div class="display-6 fw-bold"><?=($totalAmount > 0) ? round(($totalPaid/$totalAmount)*100) : 0?>%</div>
          <p class="mb-0 opacity-75">Collected</p>
          <div class="progress mt-2" style="height: 8px; background: rgba(255,255,255,0.3);">
            <div class="progress-bar bg-success" style="width: <?=($totalAmount > 0) ? round(($totalPaid/$totalAmount)*100) : 0?>%"></div>
          </div>
          <small class="mt-2 d-block opacity-75">
            RWF <?=number_format($totalPaid, 2)?> / RWF <?=number_format($totalAmount, 2)?>
          </small>
        </div>
      </div>
    </div>
  ...
  <div class="card shadow-sm border-0">
    <div class="table-responsive">
      <table class="table table-bordered table-hover align-middle mb-0" id="creditSalesTable">
        ...
        ?>
          <tr>
            <td colspan="<?=isset($_SESSION['user']) && $_SESSION['user']['role'] === 'manager' ? '12' : '11'?>" class="text-center text-muted py-4">No credit sales found.</td>
          </tr>
        <?php else: ?>
          <?php foreach($grouped_sales as $group): ?>
            ...
            <tr class="sale-row" data-status="<?=$status?>">
              ...
              <td class="<?=$amountPaid > 0 ? 'text-success fw-bold' : 'text-muted'?>">
                RWF <?=number_format($amountPaid,2)?>
              </td>
              ...
              <td>
                <?php if($balance > 0): ?>
                  ...
                  <button type="button" 
                          ...
                          data-bs-target="#payModal<?=$s['credit_id']?>" 
                          title="Record Payment">
                    <i class="fa fa-money-bill"></i> Pay
                  </button>
                  
                  <!-- Payment Modal -->
                  <div class="modal fade" id="payModal<?=$s['credit_id']?>" tabindex="-1">
                    <div class="modal-dialog">
                      <div class="modal-content">
                        <form method="POST" action="?r=payment/record" id="paymentForm<?=$s['credit_id']?>" data-validate-form>
                          <div class="modal-header bg-primary text-white">
                            <h5 class="modal-title"><i class="fa fa-money-bill"></i> Record Payment - Credit #<?=$s['credit_id']?></h5>
                            ...
                            <div class="alert alert-info">
                              <div class="row">
                                <div class="col-6">
                                  ...
                                  <?=htmlspecialchars($group['client_name'])?>
                                </div>
                                <div class="col-6">
                                  <strong>Product:</strong><br>
                                  <?=isset($productNames[$s['product_id']]) ? htmlspecialchars($productNames[$s['product_id']]) : $s['product_id']?>
                                </div>
                              </div>
                              <hr>
                              <div class="row mt-2">
                                <div class="col-4">
                                  ...
                                  <span class="text-primary fs-6">RWF <?=number_format($s['total_price'],2)?></span>
                                </div>
                                <div class="col-4">
                                  <strong>Amount Paid:</strong><br>
                                  <span class="text-success fs-6">RWF <?=number_format($amountPaid,2)?></span>
                                </div>
                                <div class="col-4">
                                  <strong>Balance:</strong><br>
                                  <span class="text-danger fs-5 fw-bold">RWF <?=number_format($balance,2)?></span>
                                </div>
                              </div>
                            </div>
                            
                            <div class="mb-3">
                              <label class="form-label fw-600"><i class="fa fa-money-bill"></i> Payment Amount <span class="text-danger">*</span></label>
                              <div class="input-group">
                                ...
                                <input type="number" 
                                       ...
                                       min="0.01" 
                                       max="<?=$balance?>" 
                                       data-validate="required|positiveNumber|min:0.01|max:<?=$balance?>"
                                       required 
                                       placeholder="Enter amount (Max: <?=number_format($balance,2)?>)">
                              </div>
                              <div class="invalid-feedback"></div>
                              <small class="text-muted"><i class="fa fa-info-circle"></i> You can pay partial amount or full balance (Max: RWF <?=number_format($balance,2)?>)</small>
                            </div>
                            
                            <div class="mb-3">
                              <label class="form-label fw-600"><i class="fa fa-comment"></i> Remarks</label>
                              <div class="input-group">
                                ...
                                <textarea name="remarks" 
                                          ...
                                          data-validate="maxLength:255"></textarea>
                              </div>
                              <div class="invalid-feedback"></div>
                              <small class="text-muted"><i class="fa fa-info-circle"></i> Optional - Add payment notes if needed</small>
                            </div>
                          </div>
            ...
            <?php if($status === 'rejected' && !empty($s['rejection_reason'])): ?>
              <tr class="table-warning sale-row" data-status="<?=$status?>">
                <td colspan="<?=isset($_SESSION['user']) && $_SESSION['user']['role'] === 'manager' ? '12' : '11'?>">
            ...
            <tr class="subtotal-row" style="background-color: #fff3e0; border-left: 4px solid #ef6c00;" data-client="<?=htmlspecialchars($group['client_name'])?>">
              ...
              <td class="fw-bold text-success" style="font-size: 0.9rem;">
                <i class="fa fa-wallet"></i> RWF <?=number_format($group['total_paid'],2)?>
              </td>
              <td class="fw-bold text-danger" style="font-size: 0.9rem;">
                <i class="fa fa-hand-holding-usd"></i> RWF <?=number_format($group['total_balance'],2)?>
              </td>
          ...
          <tr class="grand-total-row" style="background-color: #e8f5e9; border-left: 4px solid #2e7d32;">
            ...
            <td class="fw-bold fs-5 text-success">
              <i class="fa fa-check-circle"></i> RWF <?=number_format($totalPaid,2)?>
            </td>
            <td class="fw-bold fs-5 text-danger">
              <i class="fa fa-exclamation-circle"></i> RWF <?=number_format($totalBalance,2)?>
            </td>
            <td colspan="<?=isset($_SESSION['user']) && $_SESSION['user']['role'] === 'manager' ? '4' : '3'?>"></td>
          </tr>
        <?php endif; ?>
