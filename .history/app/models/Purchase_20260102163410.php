...
class Purchase extends Model {
    ...
    public function allGroupedBySupplier($start, $end) {
        $stmt = $this->db->prepare('SELECT p.*, s.name as supplier_name, s.supplier_id, pr.name as product_name FROM purchases p JOIN suppliers s ON p.supplier_id = s.supplier_id JOIN products pr ON p.product_id = pr.product_id WHERE p.purchase_date BETWEEN ? AND ? ORDER BY s.name, p.purchase_date DESC');
        $stmt->execute([$start, $end]);
