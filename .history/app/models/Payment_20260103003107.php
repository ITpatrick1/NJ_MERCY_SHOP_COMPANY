    ...
    public function recordPayment($credit_id, $amount_paid, $recorded_by, $remarks = '', $payment_date = null) {
        ...
        try {
            ...
            if ($balance <= 0) {
                ...
                // Partially paid - keep as overdue if past due date, otherwise partial_paid
                $current_status = $credit['status'];
                $due_date = $credit['due_date'];
                $is_overdue = strtotime($due_date) < strtotime(date('Y-m-d'));
                
                $new_status = ($is_overdue || $current_status === 'overdue') ? 'overdue' : 'partial_paid';
                $stmt = $this->db->prepare('UPDATE credit_sales SET status = ? WHERE credit_id = ?');
                $stmt->execute([$new_status, $credit_id]);
            }
