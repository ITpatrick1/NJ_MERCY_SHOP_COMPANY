      ...
      <table class="table table-bordered table-hover align-middle mb-0" id="creditSalesTable">
        ...
        <?php else: ?>
          <?php foreach($grouped_sales as $group): ?>
            ...
            <?php endforeach; ?>
            <!-- Client Subtotal Row -->
            <tr class="subtotal-row" style="background-color: #fff3e0; border-left: 4px solid #ef6c00;">
...
function filterTable(status) {
  ...
  setTimeout(() => {
    ...
    if (status === 'all') {
      ...
      rows.forEach(row => {
        row.style.display = '';
        // Calculate total for all visible rows
        const amountCell = row.querySelector('td:nth-child(6)');
        if (amountCell && !row.querySelector('strong')) {
      ...
      // Show/hide client headers and recalculate subtotals
      headers.forEach((header, index) => {
        ...
        while (currentRow && !currentRow.classList.contains('client-header-row')) {
          if (currentRow.classList.contains('sale-row')) {
            ...
            // Calculate client subtotal for visible rows
            if (currentRow.style.display !== 'none') {
              const amountCell = currentRow.querySelector('td:nth-child(6)');
              if (amountCell && !currentRow.querySelector('strong')) {
                const amount = parseFloat(amountCell.textContent.replace(/[^0-9.-]+/g, ''));
                if (!isNaN(amount)) {
                  clientTotal += amount;
                }
        ...
        // Update corresponding subtotal
        const subtotal = subtotals[index];
        if (subtotal) {
          subtotal.style.display = hasVisibleRows ? '' : 'none';
          // Update subtotal amount
          if (hasVisibleRows) {
            const subtotalAmountCell = subtotal.querySelector('td:nth-child(6)');
            if (subtotalAmountCell) {
              subtotalAmountCell.innerHTML = '<span class="fw-bold" style="color: #e65100;">RWF ' + clientTotal.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) + '</span>';
            }
          }
        }
      });
    ...
    // Update grand total
    if (grandTotalRow) {
      const grandTotalCell = grandTotalRow.querySelector('td:nth-child(6)');
      if (grandTotalCell) {
        grandTotalCell.innerHTML = '<span class="fw-bold fs-5" style="color: #1b5e20;">RWF ' + filteredTotal.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) + '</span>';
      }
      
      // Update grand total label to show filter info
      const grandTotalLabel = grandTotalRow.querySelector('td:nth-child(5)');
      if (grandTotalLabel) {
        if (status === 'all') {
          grandTotalLabel.innerHTML = '<span class="text-end fw-bold fs-5" style="color: #1b5e20;">💰 GRAND TOTAL:</span>';
        } else {
          const statusName = status.charAt(0).toUpperCase() + status.slice(1);
          grandTotalLabel.innerHTML = '<span class="text-end fw-bold fs-5" style="color: #1b5e20;">💰 TOTAL (' + statusName + '):</span>';
        }
      }
    }
