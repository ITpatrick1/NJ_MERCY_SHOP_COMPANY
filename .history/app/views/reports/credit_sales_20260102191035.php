    ...
    <div class="col-md-4">
      <div class="card stat-card text-center border-0 shadow-sm h-100" data-filter="all" onclick="filterTable('all')">
        <div class="card-body">
          ...
          <div class="card-tooltip">Click to view all credit sales • <?=$totalCount?> transactions • Total value: RWF <?=number_format($totalAmount, 2)?></div>
        </div>
      </div>
    </div>
    <div class="col-md-4">
      <div class="card stat-card text-center border-0 shadow-sm h-100" data-filter="pending" onclick="filterTable('pending')">
        <div class="card-body">
          ...
          <div class="card-tooltip">Click to filter not approved credits • <?=$notApprovedCount?> pending approval • <?=($totalCount > 0) ? round(($notApprovedCount/$totalCount)*100) : 0?>% of total</div>
        </div>
      </div>
    </div>
    <div class="col-md-4">
      <div class="card stat-card text-center border-0 shadow-sm h-100" data-filter="approved" onclick="filterTable('approved')">
        <div class="card-body">
          ...
          <div class="card-tooltip">Click to filter approved credits • <?=$approvedCount?> approved • <?=($totalCount > 0) ? round(($approvedCount/$totalCount)*100) : 0?>% of total</div>
        </div>
      </div>
    </div>
  </div>
    ...
    <div class="col-md-6">
      <div class="card stat-card text-center border-0 shadow-sm h-100" data-filter="overdue" onclick="filterTable('overdue')">
        <div class="card-body">
          ...
          <div class="card-tooltip">Click to filter overdue credits • <?=$overdueCount?> overdue • Requires immediate attention • <?=($totalCount > 0) ? round(($overdueCount/$totalCount)*100) : 0?>% of total</div>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="card stat-card text-center border-0 shadow-sm h-100" data-filter="settled" onclick="filterTable('settled')">
        <div class="card-body">
          ...
          <div class="card-tooltip">Click to filter settled credits • <?=$settledCount?> completed • <?=($totalCount > 0) ? round(($settledCount/$totalCount)*100) : 0?>% of total</div>
        </div>
      </div>
    </div>
  </div>
...
function filterTable(status) {
  ...
  setTimeout(() => {
    ...
    if (status === 'all') {
      ...
      // Show/hide client headers and subtotals based on visible rows
      headers.forEach((header, index) => {
        ...
        // Hide corresponding subtotal if no visible rows
