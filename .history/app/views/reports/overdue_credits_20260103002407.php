                ...
                <td class="text-center">
                  <div class="btn-group btn-group-sm">
                    ...
                    <button class="btn btn-outline-success" title="Record Payment" 
                            onclick="openPaymentModal(<?=$o['credit_id']?>, '<?=htmlspecialchars($o['client_name'], ENT_QUOTES)?>', <?=$o['total_price']?>)">
      ...
      <form method="POST" action="?r=payment/record" id="paymentForm">
        <div class="modal-body">
          ...
          <div class="alert alert-info">
            <strong>Client:</strong> <span id="modal_client_name"></span><br>
            <strong>Total Owed:</strong> <span id="modal_total_owed"></span>
          </div>
...
<script>
function openPaymentModal(creditId, clientName, totalOwed) {
  document.getElementById('modal_credit_id').value = creditId;
  document.getElementById('modal_client_name').textContent = clientName;
  document.getElementById('modal_total_owed').textContent = 'RWF ' + totalOwed.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2});
  
  // Set max amount to total owed
  document.getElementById('amount').max = totalOwed;
...
document.getElementById('paymentForm')?.addEventListener('submit', function(e) {
  ...
  if (amount > maxAmount) {
    e.preventDefault();
    alert('Payment amount cannot exceed the total owed amount of RWF ' + maxAmount.toFixed(2));
    return false;
  }
});
</script>
