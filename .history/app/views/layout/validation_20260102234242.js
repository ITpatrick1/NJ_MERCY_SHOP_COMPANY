...
class FormValidator {
  ...
  validateField(field) {
    if (!field.name || field.type === 'submit' || field.type === 'button' || field.type === 'hidden') return true;
  ...
  getFieldRules(field) {
    ...
    // HTML5 attributes - only add if not already in data-validate
    const existingRules = rules.join('|');
    
    if (field.hasAttribute('required') && !existingRules.includes('required')) {
      rules.push('required');
    }
    if (field.type === 'email' && !existingRules.includes('email')) {
      rules.push('email');
    }
    if (field.type === 'number' && !existingRules.includes('number')) {
      rules.push('number');
    }
    
    // Only check these if they actually have values
    ...
    if (field.hasAttribute('pattern') && field.getAttribute('pattern')) {
