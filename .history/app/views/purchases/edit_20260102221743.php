...
<div class="container my-4">
  ...
  <?php if(!empty($error)): ?>
    <div class="alert alert-danger alert-dismissible fade show" role="alert">
      ...
      <?php if(strpos($error, 'Supplier TIN') !== false): ?>
        <hr>
        <p class="mb-2"><strong>Available Suppliers:</strong></p>
        <ul class="mb-0">
          <?php foreach(array_slice($suppliers, 0, 5) as $s): ?>
            <li><strong><?= htmlspecialchars($s['name']) ?></strong> - TIN: <code><?= htmlspecialchars($s['tin_number']) ?></code></li>
          <?php endforeach; ?>
          <?php if(count($suppliers) > 5): ?>
            <li><em>...and <?= count($suppliers) - 5 ?> more</em></li>
          <?php endif; ?>
        </ul>
      <?php endif; ?>
      
      <?php if(strpos($error, 'Product') !== false): ?>
        <hr>
        <p class="mb-2"><strong>Available Products:</strong></p>
        <ul class="mb-0">
          <?php foreach(array_slice($products, 0, 5) as $p): ?>
            <li><code><?= htmlspecialchars($p['name']) ?></code></li>
          <?php endforeach; ?>
          <?php if(count($products) > 5): ?>
            <li><em>...and <?= count($products) - 5 ?> more</em></li>
          <?php endif; ?>
        </ul>
      <?php endif; ?>
    </div>
        ...
        <div class="row g-3">
          <div class="col-md-6">
            ...
            <input type="text" 
                   name="supplier_tin" 
                   class="form-control" 
                   list="supplier-tin-list" 
                   value="<?= htmlspecialchars($purchase['supplier_tin'] ?? '') ?>"
                   placeholder="Enter or select supplier TIN"
                   required>
            <datalist id="supplier-tin-list">
            ...
            <small class="text-muted">
              <i class="fa fa-info-circle"></i> Type or select from existing suppliers
            </small>
          ...
          <div class="col-md-6">
            ...
            <small class="text-muted">
              <i class="fa fa-info-circle"></i> Type or select from existing products
            </small>
