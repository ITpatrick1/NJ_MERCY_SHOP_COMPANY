...
class ReportController extends Controller {
    ...
    public function creditSales(){
        $this->ensure();
        $pdo = Database::pdo();
        $stmt = $pdo->query("SELECT cs.*, c.name as client_name, c.tin_number as client_tin, c.phone as client_phone, c.client_id FROM credit_sales cs JOIN clients c ON cs.client_id = c.client_id ORDER BY c.name, cs.date_issued DESC");
        $sales = $stmt->fetchAll();
        ...
        foreach($sales as $s) {
            ...
            if ($status === 'overdue') {
                ...
                // Active credits (pending, approved, or any other non-overdue/non-settled status)
                $activeCount++;
                $activeAmount += $s['total_price'];
            }
        }
