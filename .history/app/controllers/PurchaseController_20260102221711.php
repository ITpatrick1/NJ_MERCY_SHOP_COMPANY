...
class PurchaseController extends Controller {
    ...
    public function edit() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            // Resolve supplier by TIN if provided
            if (!empty($supplier_tin)) {
                $foundSupplier = $supplierModel->findByTin($supplier_tin);
                if ($foundSupplier) {
                    $supplier_id = $foundSupplier['supplier_id'];
                } else {
                    $error = 'Supplier TIN "' . htmlspecialchars($supplier_tin) . '" not found in database. Please check the TIN or select from the suggestions.';
                }
            }

            // Resolve product by name if provided
            if (!empty($product_name) && empty($error)) {
                $foundProduct = $productModel->findByName($product_name);
                if ($foundProduct) {
                    $product_id = $foundProduct['product_id'];
                } else {
                    $error = 'Product "' . htmlspecialchars($product_name) . '" not found in database. Please check the name or select from the suggestions.';
                }
            }
