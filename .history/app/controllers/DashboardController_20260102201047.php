...
class DashboardController extends Controller {
    ...
    private function getRecentActivity($limit = 10) {
        ...
        // Add icon and color for each activity type
        foreach ($activities as &$activity) {
            ...
            if (strpos($action, 'login') !== false) {
                ...
                $activity['color'] = 'info';
            } elseif (strpos($action, 'credit') !== false || strpos($action, 'sale') !== false) {
                $activity['icon'] = 'fa-credit-card';
                $activity['color'] = 'primary';
            } elseif (strpos($action, 'payment') !== false) {
                $activity['icon'] = 'fa-money-bill-wave';
                $activity['color'] = 'success';
            } elseif (strpos($action, 'product') !== false) {
                $activity['icon'] = 'fa-box';
                $activity['color'] = 'warning';
            } elseif (strpos($action, 'purchase') !== false) {
                $activity['icon'] = 'fa-shopping-cart';
                $activity['color'] = 'secondary';
            } elseif (strpos($action, 'expense') !== false) {
                $activity['icon'] = 'fa-receipt';
                $activity['color'] = 'danger';
            } elseif (strpos($action, 'client') !== false) {
                $activity['icon'] = 'fa-user-plus';
                $activity['color'] = 'info';
