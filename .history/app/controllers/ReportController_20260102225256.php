...
class ReportController extends Controller {
    ...
    public function profit() {
        ...
        // Get user input for total capital
        $totalCapitalInput = isset($_POST['total_capital']) ? floatval($_POST['total_capital']) : (isset($_SESSION['total_capital']) ? $_SESSION['total_capital'] : 0);
        
        // Store in session for persistence and redirect to avoid form resubmission
        if (isset($_POST['total_capital'])) {
            $_SESSION['total_capital'] = $totalCapitalInput;
            // Redirect to GET request to avoid form resubmission issues
            redirect('?r=report/profit&month=' . urlencode($month));
            return;
        }
        ...
        // Calculate total capital invested (all-time purchases)
        $stmt = $pdo->query("SELECT COALESCE(SUM(quantity * unit_price), 0) as total FROM purchases");
        $totalCapitalInvested = $stmt->fetch()['total'];
        
        // Calculate total sales revenue for the month (from daily_sales)
        $dailySalesTotal = 0;
        try {
            $stmt = $pdo->prepare("SELECT COALESCE(SUM(amount), 0) as total FROM daily_sales WHERE DATE_FORMAT(sales_date, '%Y-%m') = ?");
            $stmt->execute([$month]);
            $dailySalesTotal = $stmt->fetch()['total'];
        } catch (Exception $e) {
            // Table doesn't exist yet
            $dailySalesTotal = 0;
        }
        
        // Calculate total credit sales revenue for the month (only paid/settled)
        $creditSalesTotal = 0;
        try {
            $stmt = $pdo->prepare("SELECT COALESCE(SUM(amount_paid), 0) as total FROM credit_sales WHERE DATE_FORMAT(date_issued, '%Y-%m') = ?");
            $stmt->execute([$month]);
            $creditSalesTotal = $stmt->fetch()['total'];
        } catch (Exception $e) {
        ...
        $totalSalesMonth = $dailySalesTotal + $creditSalesTotal;
        
        // Calculate yesterday's sales (daily_sales only)
        $yesterday = date('Y-m-d', strtotime('-1 day'));
        $yesterdaySales = 0;
        try {
            $stmt = $pdo->prepare("SELECT COALESCE(SUM(amount), 0) as total FROM daily_sales WHERE sales_date = ?");
        ...
        // Calculate yesterday's credit payments
        $yesterdayCreditPayments = 0;
        try {
            $stmt = $pdo->prepare("SELECT COALESCE(SUM(amount_paid), 0) as total FROM credit_payments WHERE payment_date = ?");
            $stmt->execute([$yesterday]);
            $yesterdayCreditPayments = $stmt->fetch()['total'];
        } catch (Exception $e) {
            $yesterdayCreditPayments = 0;
        }
        
        $yesterdayTotal = $yesterdaySales + $yesterdayCreditPayments;
        
        // Calculate overdue credits - with dummy data fallback
        $overdueCredits = [];
        $overdueCount = 0;
        $overdueAmount = 0;
        try {
            $stmt = $pdo->query("SELECT cs.*, c.name as client_name, c.phone FROM credit_sales cs JOIN clients c ON cs.client_id = c.client_id WHERE cs.due_date < CURDATE() AND cs.status IN ('active', 'overdue', 'pending') ORDER BY cs.due_date ASC LIMIT 5");
            $overdueCredits = $stmt->fetchAll();
            
            $stmt = $pdo->query("SELECT COUNT(*) as count, COALESCE(SUM(total_price - amount_paid), 0) as total FROM credit_sales WHERE due_date < CURDATE() AND status IN ('active', 'overdue', 'pending')");
            $overdueData = $stmt->fetch();
            $overdueCount = $overdueData['count'];
            $overdueAmount = $overdueData['total'];
        } catch (Exception $e) {
            // Use dummy data if tables don't exist
            $overdueCredits = [
                [
                    'credit_id' => 1,
                    'client_name' => 'MUGABO Jean',
                    'phone' => '+250788123456',
                    'total_price' => 350000,
                    'amount_paid' => 100000,
            ...
            $overdueCount = 3;
            $overdueAmount = 700000; // (350000-100000) + (180000-50000) + (520000-200000)
        }
        
        // Calculate total money used (expenses + purchases for the month)
        $totalMoneyUsed = $expensesTotal + $purchasesTotal;
        
        // Calculate available money (sales - money used)
        $availableMoney = $totalSalesMonth - $totalMoneyUsed;
        
        // Calculate actual money remaining (if capital was input)
        $actualMoneyRemaining = $totalCapitalInput > 0 ? ($totalCapitalInput - $totalMoneyUsed + $totalSalesMonth) : 0;
        
        $this->view('reports/profit', compact(
            'expensesTotal', 
            'purchasesTotal', 
            'month',
