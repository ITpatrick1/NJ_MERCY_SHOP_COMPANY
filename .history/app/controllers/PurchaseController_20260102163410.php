...
class PurchaseController extends Controller {
    ...
    public function index() {
        ...
        $grouped_purchases = $purchaseModel->allGroupedBySupplier($start, $end);
        ...
        $this->view('purchases/index', compact('grouped_purchases', 'grand_total', 'start', 'end'));
    }
    ...
    public function edit() {
        ...
        $id = $_GET['id'] ?? null;
        if (!$id) {
            $this->setFlash('Invalid purchase ID.', 'danger');
            redirect('?r=purchase/index');
        }
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (!$supplier_id || !$product_id || $quantity <= 0 || $unit_price < 0) {
